...
export function normalizeConfig(raw) {
    ...
    return {
        regions: Array.isArray(obj.regions)
            ? obj.regions.map((r) => String(r))
            : undefined,
        okinawa: coercePlacement(obj.okinawa),
        drawOmissionLine: coerceBoolean(obj.drawOmissionLine ?? obj.draw_omission_line, true),
        theme: {
            fillDefault: coerceString(themeRaw.fillDefault ?? themeRaw.fill_default, baseTheme.fillDefault),
            borderColor: coerceString(themeRaw.borderColor ?? themeRaw.border_color, baseTheme.borderColor),
            showBorders: coerceBoolean(themeRaw.showBorders ?? themeRaw.show_borders, baseTheme.showBorders),
            labelColor: coerceString(themeRaw.labelColor ?? themeRaw.label_color, baseTheme.labelColor),
        },
        size: {
            width: coerceOptionalString(sizeRaw.width ?? sizeRaw.map_width ?? obj.map_width),
            height: coerceOptionalString(sizeRaw.height ?? sizeRaw.map_height ?? obj.map_height),
        },
    };
}
export function geometryFromConfig(config) {
    const filtered = filterRegions(PREF_GEOMETRY.paths, config.regions);
    const adjusted = config.okinawa === "original"
...
export class MapView {
    ...
    updateData(payload) {
        ...
        this.renderOmissionLine(payload.config, payload.geometry.okinawaLine ?? null);
    ...
    bootstrapFromConfig(config, inputId) {
        if (inputId) {
            this.inputId = inputId;
        }
    ...
    ensureSvg() {
        ...
        if (this.svg) {
            this.svg.setAttribute("viewBox", this.viewBox.map((v) => formatNumber(v)).join(" "));
            this.applySize();
        }
    }
    applySize() {
    ...
    applyTooltips(tooltips) {
        this.pathMap.forEach((path, code) => {
            const tip = tooltips[code] ?? path.dataset.defaultTip ?? code;
            let existing = path.querySelector("title");
            if (!existing) {
    ...
    renderOmissionLine(config, line) {
        if (!this.svg)
            return;
        const shouldDraw = line &&
        ...
        if (!this.omissionLine) {
            this.omissionLine = document.createElementNS("http://www.w3.org/2000/svg", "line");
    ...
    renderColorbar(fills, style) {
        ...
        if (!this.colorbarGroup) {
            this.colorbarGroup = document.createElementNS("http://www.w3.org/2000/svg", "g");
...
function filterRegions(paths, regions) {
...
function repositionOkinawa(paths, placement) {
    ...
    let line;
    if (mainland) {
        const mid = placement === "topleft"
            ? [newBBox.maxX, newBBox.maxY]
            : [newBBox.minX, newBBox.minY];
        const availableNE = Math.min(mainland.maxX - mid[0], mid[1] - mainland.minY);
        const availableSW = Math.min(mid[0] - mainland.minX, mainland.maxY - mid[1]);
        const length = Math.max(0, Math.min(availableNE, availableSW));
        const origin = [mid[0] - length, mid[1] + length];
        const target = [mid[0] + length, mid[1] - length];
        line = { origin, target };
    }
    return {
...
function coercePlacement(value) {
