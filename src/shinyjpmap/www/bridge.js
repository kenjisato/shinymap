...
import { MapView, normalizeConfig, } from "./map_view.js";
const views = new Map();
...
function registerHandler(id) {
    if (!id || registeredHandlers.has(id))
        return;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const Shiny = window.Shiny;
    if (!Shiny || !Shiny.addCustomMessageHandler)
        return;
    Shiny.addCustomMessageHandler(`jpmap:set_data:${id}`, (payload) => onSetData(id, payload));
...
function bootstrapOutputs(scope = document) {
    scope.querySelectorAll(".jpmap-output").forEach((el) => {
        queueHandler(el.id);
    });
}
function bootstrapInputs(scope = document) {
    scope.querySelectorAll(".jpmap-input").forEach((el) => {
        if (!el.id || initializedInputs.has(el.id))
            return;
        const view = ensureView(el);
...
function readConfig(el) {
    const container = el.closest(".jpmap");
